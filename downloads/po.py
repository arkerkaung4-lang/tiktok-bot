...
def callback_handler(call):
    ...
    if call.data == "btn_scan" or call.data == "btn_find":
        ...
        bot.send_message(call.message.chat.id, "🔍 ဥပမာ - `/find @hacker24` (သို့) `/find 8746948568` ဟု ရိုက်ထည့်ပါ။", parse_mode="Markdown")
...
def find_info(message):
    ...
    # Bronx API Endpoint အသစ်သို့ ချိတ်ဆက်ခြင်း (id parameter ဖြင့် query ပို့မည်)
    api_url = f"https://bronx-web-api.onrender.com/api/custom/telegram-scan?key=demo-key&id={query}"
    ...
    try:
        ...
        # API Response တွင် Data မရှိခြင်း သို့မဟုတ် error တက်ခြင်းကို စစ်ဆေးခြင်း
        ...
        if not data:
            ...
            if data.get("success") is False:
                is_empty_result = True
            elif "error" in data or "not found" in str(data).lower():
                is_empty_result = True
            elif "result" in data and not data.get("result"):
                is_empty_result = True
            elif "data" in data and not data.get("data"):
                is_empty_result = True
