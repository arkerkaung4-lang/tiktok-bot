...
# Bot အချက်အလက်များ
TOKEN = "8826375546:AAGva2QZAUVqZvNvH-NPDRXilrueZUxiICk"
OWNER_ID = 7936016365  
...
def callback_handler(call):
    ...
    if call.data == "btn_scan" or call.data == "btn_find":
        ...
        buy_text = (
            ...
            "📥 ဝယ်ယူပုံ အဆင့်ဆင့်:\n"
            "၁။ အထက်ပါ နံပါတ်သို့ လိုချင်သည့် Credit ပမာဏအလျောက် ငွေလွှဲပါ။\n"
            "၂။ ငွေလွှဲပြီးပါက ငွေလွှဲပြေစာ (Screenshot) ပုံကို ဤဘော့တ်ဆီသို့ ပုံနှင့်အတူ လိုချင်သော Credit ပမာဏ (ဥပမာ - 5) ကို Caption ရေး၍ ပို့ပါ။"
        )
    ...
    # Owner က Reply ခလုတ်နှိပ်လိုက်တဲ့အခါ
...
def fake_redeem(message):
    args = message.text.split()
    if len(args) < 3:
        bot.reply_to(message, "⚠️ သုံးပုံစံ: /fakeredeem <User_ID> <Amount>", parse_mode="Markdown")
        return
    ...
    try:
        ...
        bot.reply_to(message, f"🎁 Fake Redeem Successful!\n- User ID: {target_id}\n- Added Credits: {add_val}\n- Total Balance: {user_credits[target_id]}", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: မှားယွင်းနေသော ပုံစံ ဖြစ်နေပါသည်။")
...
def deduct_user(message):
    ...
    try:
        ...
        if val == 'all':
            ...
            bot.reply_to(message, f"✅ User ID: {target_id} ထံမှ Credit {sub_val} ကို နုတ်ယူလိုက်ပါပြီ။", parse_mode="Markdown")
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {e}")
...
def broadcast_message(message):
    ...
    bot.reply_to(message, f"✅ Broadcast ပြီးပါပြီ။ - ပို့နိုင်သူ: {success_count} ဦး")
...
def get_tg_info(message):
    ...
    if user_id != OWNER_ID and user_id not in unlimited_users:
        ...
        if current_credits <= 0:
            bot.reply_to(message, "❌ NO CREDITS\nYou have 0 credits. Please buy credits to continue.", parse_mode="Markdown")
            return

    args = message.text.split()
    if len(args) < 2:
        bot.reply_to(message, "⚠️ ဥပမာ - /info 8746948568", parse_mode="Markdown")
        return

    query = args[1]
    sent_msg = bot.reply_to(message, "🔍 အချက်အလက်များကို ရှာဖွေနေပါသည်...", parse_mode="Markdown")
    ...
    try:
        ...
        if isinstance(data, dict) and data.get("success") == False:
            result_text = "⚠️ အချက်အလက် ရှာမတွေ့ပါ။"
        else:
            if user_id != OWNER_ID and user_id not in unlimited_users:
                current_credits = user_credits.get(user_id, DEFAULT_FREE_LIMIT)
...
def handle_all_messages(message):
    ...
    # Owner ပို့တဲ့စာဆိုရင် ကျော်မည်
    if user_id == OWNER_ID:
        return
    ...
    # Owner ဆီသို့ User ရဲ့ မက်ဆေ့ဂျ်ကို ပို့မည်
...
print("Bot is running with Owner Notification & Reply System...")
