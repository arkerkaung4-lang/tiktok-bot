...
def callback_handler(call):
    ...
    if call.data == "btn_scan" or call.data == "btn_find":
        bot.answer_callback_query(call.id, "အချက်အလက်ရှာရန် /find နောက်တွင် ID သို့မဟုတ် Username ထည့်ပါ။")
        bot.send_message(call.message.chat.id, "🔍 ဥပမာ - `/find 8746948568` ဟု ရိုက်ထည့်ပါ။", parse_mode="Markdown")
...
def deduct_user(message):
    ...
    try:
        ...
        if val == 'all':
            ...
            user_credits.get(target_id, DEFAULT_FREE_LIMIT) # safe reference
...
# /info က ကိုယ့်အချက်အလက် (Info) ကိုပဲ သီးသန့်ပြပေးရန်
...
# /find က အချက်အလက်ရှာဖွေရန် သီးသန့်
...
def find_info(message):
    ...
    args = message.text.split()
    if len(args) < 2:
        bot.reply_to(message, "⚠️ ကျေးဇူးပြု၍ ရှာလိုသည့် ID သို့မဟုတ် Username ထည့်ပါ။\nဥပမာ - `/find 8746948568`", parse_mode="Markdown")
        return

    query = args[1]
    sent_msg = bot.reply_to(message, "🔍 **အချက်အလက်များကို ရှာဖွေနေပါသည်၊ ခဏစောင့်ပါ...**", parse_mode="Markdown")

    # Owner ကိုယ်တိုင် သို့မဟုတ် Obito27891 ကို ရှာပါက ရှာမတွေ့ကြောင်း အတိအကျပြရန်
    if query.lower() in ["@obito27891", "obito27891", str(OWNER_ID)] or query == "7936016365":
        result_text = (
            "⚠️ **Result:**\n\n"
    ...
    try:
        ...
        if isinstance(data, dict):
            if data.get("success") == False or not data or data.get("result") == None or data.get("data") == None:
                is_empty_result = True
            if all(v is None for v in data.values()) if data else True:
                is_empty_result = True
        elif not data:
            is_empty_result = True
        ...
        bot.edit_message_text("❌ **Error:** API ဆာဗာ ချိတ်ဆက်မှု အချိန်ကုန်သွားပါပြီ (Timeout)။ ခဏနေမှ ထပ်ကြိုးစားပါ။", chat_id=message.chat.id, message_id=sent_msg.message_id)
    except requests.exceptions.ConnectionError:
        bot.edit_message_text("❌ **Error:** ဆာဗာသို့ ချိတ်ဆက်၍မရပါ။ အင်တာနက်လိုင်း သို့မဟုတ် API လိပ်စာကို စစ်ဆေးပါ။", chat_id=message.chat.id, message_id=sent_msg.message_id)
    except Exception as e:
        bot.edit_message_text(f"❌ **API Error:** `{e}`\n\n*(ℹ️ API ဘက်တွင် အမှားအယွင်းရှိနေပါသဖြင့် သင့် Credit ကို မနှုတ်ယူပါ။)*", chat_id=message.chat.id, message_id=sent_msg.message_id, parse_mode="Markdown")
...
print("Bot is running with separated /info and /find functions...")
