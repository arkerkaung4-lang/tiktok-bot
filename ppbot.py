...
# /info Command (အချက်အလက်ရှာဖွေရန်)
...
# -------------------------------------------------------------
# User များ ပို့သမျှစာများကို အကုန်ဖမ်းယူပြီး တစ်ဦးချင်းစီ ပြန်စာပို့ရန်
# -------------------------------------------------------------
...
def handle_all_messages(message):
    ...
    # Owner က ပို့တဲ့စာဆိုရင် (တခြားသူဆီ တိုက်ရိုက် Reply ပြန်ချင်ရင် သုံးလို့ရအောင် ထည့်ထားပေးနိုင်ပါတယ် - လောလောဆယ် ကျော်ထားသည်)
    if user_id == OWNER_ID:
        return

    user_text = message.text
    
    # ဤနေရာတွင် User တစ်ဦးချင်းစီ ပို့လာသောစာကို Bot က အလိုအလျောက် တုံ့ပြန်မည့် စာသားကို ရေးနိုင်ပါသည်။
    reply_text = (
        f"📩 သင်၏ မက်ဆေ့ဂျ်ကို လက်ခံရရှိပါပြီ။\n"
        f"အချက်အလက်ရှာဖွေရန် `/info <ID or Username>` ကို အသုံးပြုပါ။\n\n"
        f"⚡ Power By {DEV_CREDIT}"
    )
    
    bot.reply_to(message, reply_text, parse_mode="Markdown")

print("Bot is running with full messaging and auto-reply support...")
